...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function loadProjects() {
    ...
    // Show loading state
    portfolioGrid.innerHTML = '<div class="loading-container"><div class="loading-spinner"></div><p>Loading projects...</p></div>';
    ...
    try {
      ...
      // Small delay to show loading state, then display projects
      setTimeout(() => {
      ...
      // Show error message
      portfolioGrid.innerHTML = '<div class="error-container"><p style="text-align: center; color: #EC3499; grid-column: 1/-1;">⚠️ Unable to load projects. Please refresh the page.</p></div>';
    }
  }
  ...
  function displayProjects(projects) {
    ...
    projects.forEach(project => {
      ...
      projectItem.innerHTML = `
        <img src="${project.coverImage}" alt="${project.title}" class="portfolio-img" loading="lazy" onerror="this.src='https://via.placeholder.com/400x250/1546C7/ffffff?text=${encodeURIComponent(project.title.replace(/[^a-zA-Z0-9\s]/g, '').substring(0, 20))}'">
  ...
  function createProjectModal(project) {
    ...
    project.frames.forEach((frame, index) => {
      const mediaHtml = frame.video 
        ...
        : `<img src="${frame.image}" alt="${frame.section}" class="modal-img" onclick="window.open(this.src, '_blank')" onerror="this.src='https://via.placeholder.com/800x400/1546C7/ffffff?text=${encodeURIComponent(frame.section.replace(/[^a-zA-Z0-9\s]/g, '').substring(0, 15))}'" title="Click to view full size">`;
